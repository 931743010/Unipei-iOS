...
//    BOOL shouldSelect = previousCount < self.maximumImagesCount;
    BOOL shouldSelect = previousCount < self.maximumImagesCount-self.currentImagesCount;
    if (!shouldSelect) {
...
//        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"You can only send %d photos at a time.", nil), self.maximumImagesCount];
        ...
        NSString *title = [NSString stringWithFormat:NSLocalizedString(@"%d张图片!", nil), previousCount];
        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"当前选择图片数量%d张，图片数量总和不能超过%d张！", nil), previousCount,self.maximumImagesCount];
