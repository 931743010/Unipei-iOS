...
- (void)viewDidLoad {
    ...
    UINib *cellNib2 = [UINib nibWithNibName:moreCellName bundle:nil];
    [_tableView registerNib:cellNib2 forCellReuseIdentifier:moreCellName];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==0) {
        NSString *idName = [NSString stringWithFormat:@"showLottery%d",indexPath.row];
...
-(UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    MoreCell  *useCell = [_tableView dequeueReusableCellWithIdentifier:moreCellName];
    [useCell.moreButton setTitle:@"立即使用" forState:UIControlStateNormal];
    return useCell;
}
