//
//  ELCImagePickerController+Helper.m
//  DymIOSApp
//
//  Created by QinJun on 11/4/15.
...
@implementation ELCImagePickerController (Helper)
...
+(BOOL)canTakePicture {
    ...
    if (![self authorizationStatus]) {
        UIAlertView * alertView = [[UIAlertView alloc] initWithTitle:@"未获得授权使用摄像头" message:@"请在iOS '设置中-隐私-相机' 中允许'由你配'访问你的相机 ！" delegate:self cancelButtonTitle:nil otherButtonTitles:@"确认", nil];
         [alertView show];
         return NO;
    }
...
+(void)pickImageWithDelegate:(id)delegate
    ...
    UIAlertAction *takePicture = [UIAlertAction actionWithTitle:@"拍照" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        if ([ELCImagePickerController canTakePicture]) {
            if ([self checkAlbumAuthorization] && [self checkCameraAuthorizationStatus] ){  //如果同时不能访问相机和相册不允许继续进行
                ...
                UIAlertView * alertView = [[UIAlertView alloc] initWithTitle:@"未获得授权使用" message:@"请在iOS '设置-隐私' 中允许'由你配'同时访问'照片'和'相机'。" delegate:self cancelButtonTitle:nil otherButtonTitles:@"确认", nil];
                [alertView show];
            }
        }
    }];
    ...
    UIAlertAction *pickFromLibrary = [UIAlertAction actionWithTitle:@"选择照片" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        if ([ELCImagePickerController canUsePhotoLibrary]) {
            ELCImagePickerController *elcImagePicker = [[ELCImagePickerController alloc] init];
            elcImagePicker.maximumImagesCount = 100; //Set the maximum number of images to select, defaults to 4
            elcImagePicker.imagePickerDelegate = delegate;
            elcImagePicker.maximumImagesCount = maxPhotoCount;
            elcImagePicker.currentImagesCount = currentCount;
            //Present modally
            [presentingVC presentViewController:elcImagePicker animated:YES completion:nil];
